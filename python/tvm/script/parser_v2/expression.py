...
# under the License.
"""Syntax-only rewriting of registered expression-string arguments.
...
def rewrite_expression(node, resolve, namespace, filename, *, annotation=False):
    """Copy an expression and rewrite registered expression-string fields.
    ...
    class Rewrite(ast.NodeTransformer):
        ...
        def visit_Call(self, current):
            ...
            policy = protocol.expr_str_policy(constructor)
            parameters = []
            ...
            current.func = self.visit(current.func)
            current.args = [
                self.field(value, policy)
                if index < len(parameters) and parameters[index] in policy.fields
                else self.visit(value)
                for index, value in enumerate(current.args)
            ]
            for keyword in current.keywords:
                keyword.value = (
                    self.field(keyword.value, policy)
                    if policy and keyword.arg in policy.fields
