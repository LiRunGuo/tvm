...
class IRBuilderTranspiler(ast.NodeTransformer):
    ...
    def _expression(self, original, *, attach_span=True):
        ...
        node = rewrite_expression(node, self._resolve, self.dialect_prefix, self.filename)
        ...
        # every operand left-to-right while building; compiled IR is conditional.
    ...
    def _compare_chain(self, node):
        ...
        # once in source order. It passes ordinary overloaded comparisons to the
        # builder, never lazy callbacks. chain=(a, b, c) records syntax provenance so
    ...
    def visit_AnnAssign(self, node):
        ...
        annotation = rewrite_expression(
            node.annotation, self._resolve, self.dialect_prefix, self.filename, annotation=True
        )
    ...
    def visit_Expr(self, node):
        # Pattern: expression -> X.emit_(translated_expression).
    ...
    def visit_If(self, node):
        # Pattern: if condition: ... else: ... -> host branch or X.If/Then/Else plus named exports.
        ...
        if self.preserve_return:
            ...
            native.append(
                self._assign(
                    mapping_name,
                    self._name(frame, node),
                    node,
                )
            )
            tail = self._exports(mapping_name, candidates, node)
    ...
    def function_program(self, node, runtime, bindings, *, local=False):
        ...
        record_expr = self._call(
            runtime,
            "FunctionRecord",
            [self._name(builder, node), ast.Constant(node.name), options, self.span(node)],
            node,
        ...
        for name, annotation, location, in_body in getattr(node, "_tvm_type_var_declarations", ()):
            ...
            metadata = getattr(constructor, "__tvm_type_var_decl__", None) or getattr(
                constructor, "__tvm_declaration_args__", None
            )
        ...
        for parameter in node.args.args:
            ...
            annotation = rewrite_expression(
                parameter.annotation, self._resolve, builder, self.filename, annotation=True
            )
        ...
        if node.returns is not None:
            annotation = rewrite_expression(
                node.returns, self._resolve, builder, self.filename, annotation=True
            )
            ...
            declaration.append(
                self._statement(
                    self._call(record, "returns", [self._expression(annotation)], node.returns),
                    node.returns,
                )
            )
        ...
        old_bound, old_optional, old_environment, old_nested = (
            self.bound,
            self.optional,
            self.namespace_bindings,
        ...
        self.bound, self.optional, self.namespace_bindings, self.nested_function = (
            old_bound,
            old_optional,
            old_environment,
            old_nested,
    ...
    def program(self, tree, runtime, original_name, bindings):
        ...
        for member in members:
            ...
            body.append(copy.deepcopy(member))
            targets = (
                member.targets
